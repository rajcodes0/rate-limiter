...
function App() {
    ...
    <section className="layout">
      <aside className="controls"><label>Algorithm<select value={algorithm} onChange={(e) => { setAlgorithm(e.target.value); setEvents([]); }}>{Object.entries(labels).map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label>
      ...
      <section className="visual"><div className="stats"><article><span>Allowed</span><strong>{allowed}</strong></article><article><span>Rejected</span><strong>{denied}</strong></article><article><span>Remaining</span><strong>{events.at(-1)?.remaining ?? limit}</strong></article></div>
        <div className={`bucket ${algorithm}`}><div className="bucket-title">{labels[algorithm]}</div><div className="slots">{Array.from({ length: Math.min(capacity, 20) }, (_, index) => <i key={index} className={index < (events.at(-1)?.remaining ?? capacity) ? 'filled' : ''}/>)}</div><p>{bucketAlgorithm ? 'Filled cells show currently available capacity.' : 'Each mark below is a real response from the API.'}</p></div>
